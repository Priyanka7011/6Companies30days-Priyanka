 int splitArray(vector<int>& nums, int m) {
    ...
    /*
    whenever we have to minimise the largest or maximize the smallest
    then we use binary search.

    we will reduce the end if we get less partitions than required so that next time we get less 
    partition
    and increase the start if we get more partition 
